"""
ldapobject.py - wraps class _pyldap.LDAPObject
...
- LDAPObject class should be exactly the same like _pyldap.LDAPObject
...
import sys,time,pprint,_pyldap,pyldap,pyldap.sasl,pyldap.functions

from pyldap.schema import SCHEMA_ATTRS
from pyldap.controls import LDAPControl,DecodeControlTuples,RequestControlTuples
from pyldap.extop import ExtendedRequest,ExtendedResponse

from pyldap import LDAPError


class NO_UNIQUE_ENTRY(pyldap.NO_SUCH_OBJECT):
  """
...
class SimpleLDAPObject:
  """
  Drop-in wrapper class around _pyldap.LDAPObject
  """

  CLASSATTR_OPTION_MAPPING = {
    "protocol_version":   pyldap.OPT_PROTOCOL_VERSION,
    "deref":              pyldap.OPT_DEREF,
    "referrals":          pyldap.OPT_REFERRALS,
    "timelimit":          pyldap.OPT_TIMELIMIT,
    "sizelimit":          pyldap.OPT_SIZELIMIT,
    "network_timeout":    pyldap.OPT_NETWORK_TIMEOUT,
    "error_number":pyldap.OPT_ERROR_NUMBER,
    "error_string":pyldap.OPT_ERROR_STRING,
    "matched_dn":pyldap.OPT_MATCHED_DN,
  }
  ...
  def __init__(
    ...
    self._l = pyldap.functions._ldap_function_call(pyldap._ldap_module_lock,_pyldap.initialize,uri)
    self.timeout = -1
    self.protocol_version = pyldap.VERSION3

  def _ldap_lock(self,desc=''):
    if pyldap.LIBLDAP_R:
      return pyldap.LDAPLock(desc='%s within %s' %(desc,repr(self)))
    else:
      return pyldap._ldap_module_lock
      ...
      try:
        ...
        if __debug__ and self._trace_level>=2:
          if func.__name__!="unbind_ext":
            diagnostic_message_success = self._l.get_option(pyldap.OPT_DIAGNOSTIC_MESSAGE)
      finally:
  ...
  def fileno():
    ...
    Just a convenience wrapper for LDAPObject.get_option(pyldap.OPT_DESC)
    """
    return self.get_option(pyldap.OPT_DESC)
  ...
  def cancel_s(self,cancelid,serverctrls=None,clientctrls=None):
    ...
    except (pyldap.CANCELLED,pyldap.SUCCESS):
      res = None
    return res
  ...
  def bind(self,who,cred,method=pyldap.AUTH_SIMPLE):
    """
    bind(who, cred, method) -> int
    """
    assert method==pyldap.AUTH_SIMPLE,'Only simple bind supported in LDAPObject.bind()'
    return self.simple_bind(who,cred)

  def bind_s(self,who,cred,method=pyldap.AUTH_SIMPLE):
    """
  ...
  def sasl_interactive_bind_s(self,who,auth,serverctrls=None,clientctrls=None,sasl_flags=pyldap.SASL_QUIET):
    """
    sasl_interactive_bind_s(who, auth [,serverctrls=None[,clientctrls=None[,sasl_flags=pyldap.SASL_QUIET]]]) -> None
    """
    return self._ldap_call(self._l.sasl_interactive_bind_s,who,auth,RequestControlTuples(serverctrls),RequestControlTuples(clientctrls),sasl_flags)

  def sasl_non_interactive_bind_s(self,sasl_mech,serverctrls=None,clientctrls=None,sasl_flags=pyldap.SASL_QUIET):
    """
    Send a SASL bind request using a non-interactive SASL method (e.g. GSSAPI, EXTERNAL)
    """
    self.sasl_interactive_bind_s('',pyldap.sasl.sasl({},sasl_mech))

  def sasl_external_bind_s(self,serverctrls=None,clientctrls=None,sasl_flags=pyldap.SASL_QUIET):
    """
  ...
  def sasl_gssapi_bind_s(self,serverctrls=None,clientctrls=None,sasl_flags=pyldap.SASL_QUIET):
    """
  ...
  def compare_ext_s(self,dn,attr,value,serverctrls=None,clientctrls=None):
    ...
    try:
      resp_type, resp_data, resp_msgid, resp_ctrls = self.result3(msgid,all=1,timeout=self.timeout)
    except pyldap.COMPARE_TRUE:
      return 1
    except pyldap.COMPARE_FALSE:
      return 0
    return None
  ...
  def extop(self,extreq,serverctrls=None,clientctrls=None):
    ...
    extop_s(extreq[,serverctrls=None[,clientctrls=None[,extop_resp_class=None]]]]) ->
        ...
        The extreq is an instance of class pyldap.extop.ExtendedRequest.

        If argument extop_resp_class is set to a sub-class of
        pyldap.extop.ExtendedResponse this class is used to return an
  ...
  def extop_result(self,msgid=pyldap.RES_ANY,all=1,timeout=None):
  ...
  def extop_s(self,extreq,serverctrls=None,clientctrls=None,extop_resp_class=None):
    ...
    if extop_resp_class:
      ...
      if extop_resp_class.responseName!=respoid:
        raise pyldap.PROTOCOL_ERROR("Wrong OID in extended response! Expected %s, got %s" % (extop_resp_class.responseName,respoid))
  ...
  def result(self,msgid=pyldap.RES_ANY,all=1,timeout=None):
    """
  ...
  def result2(self,msgid=pyldap.RES_ANY,all=1,timeout=None):
    resp_type, resp_data, resp_msgid, resp_ctrls = self.result3(msgid,all,timeout)
    return resp_type, resp_data, resp_msgid

  def result3(self,msgid=pyldap.RES_ANY,all=1,timeout=None,resp_ctrl_classes=None):
  ...
  def result4(self,msgid=pyldap.RES_ANY,all=1,timeout=None,add_ctrls=0,add_intermediates=0,add_extop=0,resp_ctrl_classes=None):
    if timeout is None:
  ...
  def get_option(self,option):
    ...
    if option==pyldap.OPT_SERVER_CONTROLS or option==pyldap.OPT_CLIENT_CONTROLS:
  ...
  def search_subschemasubentry_s(self,dn=''):
    ...
    try:
      r = self.search_s(
        dn,pyldap.SCOPE_BASE,'(objectClass=*)',['subschemaSubentry']
      )
    except (pyldap.NO_SUCH_OBJECT,pyldap.NO_SUCH_ATTRIBUTE,pyldap.INSUFFICIENT_ACCESS):
      r = []
    except pyldap.UNDEFINED_TYPE:
      return None
    try:
      if r:
        e = pyldap.cidict.cidict(r[0][1])
  ...
  def read_s(self,dn,filterstr=None,attrlist=None,serverctrls=None,clientctrls=None,timeout=-1):
    ...
    if r:
      ...
      raise pyldap.NO_SUCH_OBJECT('Empty search result reading entry %s' % (repr(dn)))
  ...
  def find_unique_entry(self,base,scope=pyldap.SCOPE_SUBTREE,filterstr='(objectClass=*)',attrlist=None,attrsonly=0,serverctrls=None,clientctrls=None,timeout=-1):
    """
...
class NonblockingLDAPObject(SimpleLDAPObject):
  ...
  def result(self,msgid=pyldap.RES_ANY,all=1,timeout=-1):
    """
    """
...
class ReconnectLDAPObject(SimpleLDAPObject):
  """
  In case of server failure (pyldap.SERVER_DOWN) the implementations
  ...
  def __init__(
    ...
    self._reconnect_lock = pyldap.LDAPLock(desc='reconnect lock within %s' % (repr(self)))
  ...
  def _apply_last_bind(self):
    if self._last_bind!=None:
      ...
      # Send explicit anon simple bind request to provoke pyldap.SERVER_DOWN in method reconnect()
  ...
  def reconnect(self,uri,retry_max=1,retry_delay=60.0):
    ...
    try:
      ...
      while reconnect_counter:
        ...
        except (pyldap.SERVER_DOWN,pyldap.TIMEOUT):
          if __debug__ and self._trace_level>=1:
...
# pyldap.open() and pyldap.initialize()
