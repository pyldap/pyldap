"""
pyldap.schema.subentry -  subschema subentry handling
...
import pyldap.cidict,pyldap.schema

from pyldap.schema.models import *

SCHEMA_CLASS_MAPPING = pyldap.cidict.cidict()
SCHEMA_ATTR_MAPPING = {}
...
class SubSchema:
  ...
  def __init__(self,sub_schema_sub_entry,check_uniqueness=1):
    ...
    for c in SCHEMA_CLASS_MAPPING.values():
      self.name2oid[c] = pyldap.cidict.cidict()
      self.sed[c] = {}
      self.non_unique_names[c] = pyldap.cidict.cidict()

    # Transform entry dict to case-insensitive dict
    e = pyldap.cidict.cidict(sub_schema_sub_entry)
    ...
    for attr_type in SCHEMA_ATTRS:
      ...
      for attr_value in filter(None,e.get(attr_type,[])):
        ...
        if hasattr(se_instance,'names'):
          for name in pyldap.cidict.cidict({}.fromkeys(se_instance.names)).keys():
  ...
  def tree(self,schema_element_class,schema_element_filters=None):
    """
    Returns a pyldap.cidict.cidict dictionary representing the
    ...
    top_node = '_'
    tree = pyldap.cidict.cidict({top_node:[]})
  ...
  def get_obj(self,se_class,nameoroid,default=None,raise_keyerror=0):
    ...
    try:
      ...
      if raise_keyerror:
        raise KeyError('No pyldap.schema.%s instance with nameoroid %s and se_oid %s' % (
  ...
  def attribute_types(
    ...
    The attributes are stored in a pyldap.cidict.cidict dictionary.
    ...
    ignore_dit_content_rule
        ...
        is ignored
    """
    AttributeType = pyldap.schema.AttributeType
    ObjectClass = pyldap.schema.ObjectClass
    ...
    r_must,r_may = pyldap.cidict.cidict(),pyldap.cidict.cidict()
...
def urlfetch(uri,trace_level=0):
  ...
  if uri.startswith('ldap:') or uri.startswith('ldaps:') or uri.startswith('ldapi:'):
    ...
    l=pyldap.initialize(ldap_url.initializeUrl(),trace_level)
  ...
  subschemasubentry_entry = pyldap.cidict.cidict()
  ...
  if subschemasubentry_dn!=None:
    parsed_sub_schema = pyldap.schema.SubSchema(subschemasubentry_entry)
  else:
