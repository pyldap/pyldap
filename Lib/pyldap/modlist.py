"""
pyldap.modlist - create add/modify modlist's
...
import string,pyldap,pyldap.cidict
...
def list_dict(l,case_insensitive=0):
  ...
  If argument case_insensitive is non-zero pyldap.cidict.cidict will be
  used for case-insensitive string keys
  """
  if case_insensitive:
    d = pyldap.cidict.cidict()
  else:
    d = {}
  for i in l:
    d[i]=None
...
def modifyModlist(
  ...
  for attrtype in new_entry.keys():
    ...
    if not old_value and new_value:
      ...
      modlist.append((pyldap.MOD_ADD,attrtype,new_value))
      ...
      if replace_attr_value:
        modlist.append((pyldap.MOD_DELETE,attrtype,None))
        modlist.append((pyldap.MOD_ADD,attrtype,new_value))
