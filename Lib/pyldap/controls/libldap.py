...
import _pyldap,pyldap
from pyldap.controls import RequestControl,LDAPControl,KNOWN_RESPONSE_CONTROLS
...
class AssertionControl(RequestControl):
  ...
  controlType = pyldap.CONTROL_ASSERT
  def __init__(self,criticality=True,filterstr='(objectClass=*)'):
    self.criticality = criticality
    self.filterstr = filterstr

  def encodeControlValue(self):
    return _pyldap.encode_assertion_control(self.filterstr)

KNOWN_RESPONSE_CONTROLS[pyldap.CONTROL_ASSERT] = AssertionControl
...
class MatchedValuesControl(RequestControl):
  ...
  controlType = pyldap.CONTROL_VALUESRETURNFILTER
  ...
  def encodeControlValue(self):
    return _pyldap.encode_valuesreturnfilter_control(self.filterstr)

KNOWN_RESPONSE_CONTROLS[pyldap.CONTROL_VALUESRETURNFILTER] = MatchedValuesControl
...
class SimplePagedResultsControl(LDAPControl):
  ...
  """
  controlType = pyldap.CONTROL_PAGEDRESULTS
  ...
  def encodeControlValue(self):
    return _pyldap.encode_page_control(self.size,self.cookie)

  def decodeControlValue(self,encodedControlValue):
    self.size,self.cookie = _pyldap.decode_page_control(encodedControlValue)

KNOWN_RESPONSE_CONTROLS[pyldap.CONTROL_PAGEDRESULTS] = SimplePagedResultsControl
