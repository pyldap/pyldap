#!/usr/bin/env python
"""
pyldap.controls.readentry - classes for the Read Entry controls
(see RFC 4527)
...
from pyldap.controls import LDAPControl,KNOWN_RESPONSE_CONTROLS
...
class PreReadControl(ReadEntryControl):
  ...
  entry
      ...
      before the operation was done by the server
  """
  controlType = pyldap.CONTROL_PRE_READ
...
class PostReadControl(ReadEntryControl):
  ...
  entry
      ...
      after the operation was done by the server
  """
  controlType = pyldap.CONTROL_POST_READ
