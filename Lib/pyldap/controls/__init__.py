...
Description:
The pyldap.controls module provides LDAPControl classes.
...
def DecodeControlTuples(ldapControlTuples,knownLDAPControls=None):
  ...
  ldapControlTuples
      Sequence-type of 3-tuples returned by _pyldap.result4() containing
  ...
  knownLDAPControls
      ...
      pyldap.controls.KNOWN_RESPONSE_CONTROLS is used here.
  """
  ...
  for controlType,criticality,encodedControlValue in ldapControlTuples or []:
    try:
      ...
      if criticality:
        raise pyldap.UNAVAILABLE_CRITICAL_EXTENSION('Received unexpected critical response control with controlType %s' % (repr(controlType)))
    else:
...
from pyldap.controls.simple import *
from pyldap.controls.libldap import *
